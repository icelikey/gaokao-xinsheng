    ...
    <main className="adminPage">
      ...
      <section className="paperHeader">
        <div>
          ...
          <h1>{mvpPaper.title}</h1>
          <p>
            {mvpPaper.year}届 · {mvpPaper.region} · {mvpPaper.track} ·{" "}
            {mvpPaper.subject} · {mvpPaper.durationMinutes}分钟
          </p>
        </div>
        <div className="heroActions adminActions">
          <Link className="primaryButton" href={`/admin/papers/${mvpPaper.id}`}>
            预览题目
          </Link>
